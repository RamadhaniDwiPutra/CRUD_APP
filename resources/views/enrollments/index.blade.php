                ...
                <tbody>
                    @foreach($enrollments as $item)
                            <tr>
                                ...
                                <td class="text-wrap" style="max-width: 200px;">{{ $item->batch_id }}</td>
                                ...
                                <td class="d-flex flex-column flex-md-row">
                                    ...
                                    <form method="POST" action="{{ url('/enrollments' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                        {{ method_field('DELETE') }}
                                        {{ csrf_field() }}
                                            <button type="submit" class="btn btn-danger btn-sm mb-1" title="Delete Enrollment" onclick="return confirm(&quot;Confirm delete?&quot;)">
